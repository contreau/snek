<script lang="ts">
  import { speedyHighScore, currentSpeedyScore, speedyActive } from "./store";
  ...
  $: if (gameOver) {
    ...
    if ($speedyHighScore < $currentSpeedyScore)
      $speedyHighScore = $currentSpeedyScore;
  }
...
<section class="above-game">
  <div class="hud">
    <p>
      <button on:click={() => ($speedyActive = false)} class="menu-button"
        >return to menu</button
      >
    </p>
    <span>//</span>
    <p style="color: var(--speed-mode)">speedy mode</p>
  </div>
  <div class="hud">
    <p>
